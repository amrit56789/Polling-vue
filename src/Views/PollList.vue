...
<div class="max-w-4xl mx-auto py-6">
    ...
    <div v-for="(poll, index) in polls" :key="poll.id" class="mt-4 bg-gradient-to-r from-blue-100 via-blue-200 to-blue-100 rounded-lg shadow-lg px-5 py-4 hover:shadow-xl transition-shadow duration-300">
        ...
        <!-- Admin controls -->
        <div v-if="user?.roleId === 1" class="flex mt-4 justify-end">
    ...
    <button v-if="!isLoading" @click="loadMore" :disabled="polls.length<10" class="mt-6 px-6 py-3 bg-gray-200 text-gray-700 hover:bg-gray-300 w-full font-semibold rounded-lg transition-colors duration-200">
        Load More
    </button>
    <PollResultsModal :pollData="pollResultsData" :isVisible="isResultsModalVisible" @close="closeResultsModal" />
...
import PollResultsModal from './PollResultModal.vue';
export default {
    name: "PollList",
    components: {
        Icon,
    ...
    setup() {
        ...
        watchEffect(() => {
            const newVotedPolls = JSON.parse(localStorage.getItem('votedPolls')) || [];
            polls.value.forEach(poll => {
                if (newVotedPolls.includes(poll.id)) {
                    // 
                }
            });
        });
        ...
        const closeResultsModal = () => {
            isResultsModalVisible.value = false;
        };
        ...
        const submitVote = async (pollId) => {
            ...
            try {
                await submitVotes(selectedOption.value[pollId]);
